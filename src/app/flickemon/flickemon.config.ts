/**
 * Flickemon Game Data Configuration
 * ──────────────────────────────────
 * SINGLE SOURCE OF TRUTH for all game data.
 * Contains Pokémon species across Gen 1-5, starters, evolution chains,
 * encounter rates, EXP formulas, and game balance constants.
 */

// ─────────────────────────── Sprite Configuration ───────────────────────────

/** Base URL for Pokémon sprites (PokeAPI GitHub) */
export const SPRITE_BASE_URL =
    'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon';

/** Get front sprite URL for a Pokémon by National Dex ID */
...
/**
 * EXP tuning: 20 hours of study time (1,200 mins) = 1 fully evolved Pokémon (Level 36, 46,656 EXP).
 * Total target rate = 46,656 / 1,200 mins = 38.88 EXP / min.
 * - Passive Study EXP: 30 EXP / min (0.5 EXP / sec)
 * - Battle Victory Bonus: 22 EXP / win (at 2.5 mins per battle = 8.88 EXP / min)
 * Combined Total = 38.88 EXP / min (exactly 46,656 EXP at 20 hours!).
 */
export const EXP_PER_MINUTE = 30;
export const BATTLE_WIN_EXP_BONUS = 22;

/** Calculate authentic Pokémon Max HP using official Pokémon formula: Math.floor(((2 * baseHp) * level) / 100) + level + 10 */
export function calculateRealMaxHp(baseHp: number, level: number): number {
...
/**
 * Pokémon Registry — Gen 1 (All 151) + Gen 2–5 Starters & Evolutions.
 */
export const POKEMON_REGISTRY: PokemonSpecies[] = [
    // ── GEN 1: KANTO (1 - 151) ──
...
/**
 * All basic starters across Gen 1 - Gen 7 + Pikachu & Eevee!
 */
export const STARTER_OPTIONS: number[] = [
    // Gen 1
    1, 4, 7, 25, 133, // Bulbasaur, Charmander, Squirtle, Pikachu, Eevee
    // Gen 2
...
export function getSpeciesByStage(stage: 1 | 2 | 3): PokemonSpecies[] {
    return POKEMON_REGISTRY.filter(p => p.evolutionStage === stage);
}

export function getEvolution(fromId: number): EvolutionChain | undefined {
    return EVOLUTION_CHAINS.find(e => e.fromId === fromId);
}

export function getAllEvolutions(fromId: number): EvolutionChain[] {
    return EVOLUTION_CHAINS.filter(e => e.fromId === fromId);
}

export function canEvolveAt(fromId: number, level: number): EvolutionChain | null {
