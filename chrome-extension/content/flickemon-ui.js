...
class FlickemonUI {
    ...
    updateWidgetView(card, state, wild) {
        ...
        if (wild && this.lastWildHp !== undefined && wild.currentHp < this.lastWildHp && wild.status === 'fighting') {
            tookDamage = true;
        }
        this.lastWildHp = wild ? wild.currentHp : undefined;
                ...
                <div class="hud-columns">
                    ...
                    <div class="hud-col battle-col-box">
                        ${wild ? `
                            ...
                            <img src="${this.config.getSpriteUrl(wild.wildSpecies.id)}" alt="${wild.wildSpecies.name}" class="wild-mini-sprite ${wild.status} ${tookDamage ? 'damage-flash' : ''}"/>
                            <div class="battle-info">
                                ...
                                <div class="status-line ${wild.status}">
                                    ${wild.status === 'captured' ? `🏆 Captured! (+${wild.expGained || 0} EXP)` : wild.status === 'escaped' ? `💨 Escaped! (+${wild.expGained || 0} EXP)` : `⚔️ Fighting...`}
                                </div>
                            </div>
